...
export function ParticleTextEffect({ words = DEFAULT_WORDS, asBackground = false, className = "" }: ParticleTextEffectProps) {
  ...
  const animate = () => {
    ...
    // Background with motion blur (lighter for background mode)
    if (asBackground) {
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      if (asBackground) {
        // For background mode, use full viewport size
    ...
    // Mouse event handlers (only for non-background mode)
    ...
    // Handle window resize for background mode
    if (asBackground) {
