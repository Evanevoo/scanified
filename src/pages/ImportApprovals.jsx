      ...
      <Paper elevation={0} sx={{ width: '100%', p: { xs: 2, md: 5 }, borderRadius: 0, boxShadow: '0 2px 12px 0 rgba(16,24,40,0.04)', border: '1px solid #eee', bgcolor: '#fff', overflow: 'visible' }}>
        ...
        <Dialog open={detailDialog.open} onClose={() => setDetailDialog({ open: false, row: null })} maxWidth="md" fullWidth>
          ...
          <DialogContent dividers>
            {detailDialog.row && (
              <Box>
                <Typography variant="h6" mb={2}>{detailDialog.row._type === 'invoice' ? 'Invoice' : 'Sales Receipt'} #{detailDialog.row.data?.invoice_number || detailDialog.row.data?.sales_receipt_number || detailDialog.row.id}</Typography>
                <Typography variant="subtitle1" mb={1}>Customer: {detailDialog.row.data?.customer_name} ({detailDialog.row.data?.customer_id})</Typography>
                <Typography variant="subtitle2" mb={1}>Date: {detailDialog.row.data?.date}</Typography>
                <Typography variant="subtitle2" mb={2}>Product(s):</Typography>
                {Array.isArray(detailDialog.row.data?.line_items) ? (
                  <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16 }}>
                    <thead>
                      <tr>
                        <th style={{ borderBottom: '1px solid #ccc', padding: 6 }}>Product Code</th>
                        <th style={{ borderBottom: '1px solid #ccc', padding: 6 }}>Description</th>
                        <th style={{ borderBottom: '1px solid #ccc', padding: 6 }}>Qty Out</th>
                        <th style={{ borderBottom: '1px solid #ccc', padding: 6 }}>Qty In</th>
                        <th style={{ borderBottom: '1px solid #ccc', padding: 6 }}>Serial Number</th>
                        <th style={{ borderBottom: '1px solid #ccc', padding: 6 }}>Attach/Detach</th>
                      </tr>
                    </thead>
                    <tbody>
                      {detailDialog.row.data.line_items.map((item, idx) => (
                        <tr key={idx}>
                          <td style={{ padding: 6 }}>{item.product_code}</td>
                          <td style={{ padding: 6 }}>{item.description}</td>
                          <td style={{ padding: 6 }}>{item.qty_out}</td>
                          <td style={{ padding: 6 }}>{item.qty_in}</td>
                          <td style={{ padding: 6 }}>{item.serial_number}</td>
                          <td style={{ padding: 6 }}>
                            {/* Placeholder for attach/detach UI */}
                            <Button size="small" variant="outlined" sx={{ mr: 1 }}>Attach</Button>
                            <Button size="small" variant="outlined" color="error">Detach</Button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <Typography variant="subtitle2" mt={2}>Raw Data:</Typography>
                <pre style={{ background: '#f7f7f7', padding: 12, borderRadius: 6, fontSize: 13, marginTop: 8 }}>{JSON.stringify(detailDialog.row.data, null, 2)}</pre>
              </Box>
            )}
          </DialogContent>
          <DialogActions>
