...
import { formatImportDateForDisplay, parseImportDateString } from '../utils/parseImportDate';
...
import { hasExistingReturnScanForBarcode } from '../services/returnScanEndDate';
...
function expandBarcodeLookupVariants(barcodes) {
  const out = new Set();
...
async function fetchBottleScansByBarcodes(supabaseClient, organizationId, barcodeArray) {
  ...
  for (const col of ['bottle_barcode']) {
    for (const orgFilter of orgFilters) {
      ...
      if (error) logger.warn(`fetchBottleScansByBarcodes (${col}, ${orgFilter}):`, error.message);
      else addRows(data);
    }
  }
...
function shouldWarnShippedBottleOtherCustomer(bottle, targetCustomerName, targetCustomerId) {
  ...
  if (bottle.shipScanOnThisOrder && bottle.returnRecordedBeforeThisShip) return false;
  if (bottle.latestGlobalOnThisOrder) return false;
  if (bottle.returnRecordedBeforeThisShip) return false;
  if (bottle.shipScanOnThisOrder && bottle.latestScanIsThisOrderShip) return false;
...
const revertBottlesForRejectedReturn = async (orderNumber, organizationId) => {
  ...
  try {
    ...
    (bottleScansData || []).forEach(s => {
      const modeUpper = (s.mode || '').toUpperCase();
      if (s.bottle_barcode && (modeUpper === 'RETURN' || modeUpper === 'PICKUP')) barcodesToRevert.add(s.bottle_barcode);
    });
...
function determineVerificationStatus(record) {
  ...
  const hasInvoiceData = rows.some(row => {
    const shipped = parseInt(row.qty_out || row.QtyOut || row.shipped || row.Shipped || row.quantity || 0, 10);
    const returned = parseInt(row.qty_in || row.QtyIn || row.returned || row.Returned || row.return_qty || row.ReturnQty || 0, 10);
...
export default function ImportApprovals() {
  ...
  // Seed search from ?search= so global search can deep-link to a specific order
  ...
  // Keep search in sync when the omnibar navigates here with a new ?search= value
  useEffect(() => {
  ...
  async function fetchVerificationStats(silent = false, skipAutoApprove = false) {
    try {
      ...
      // Fetch imports and bottle_scans in parallel (single source of truth for scans).
      const [invoicesResult, receiptsResult, bottleScansResult] = await Promise.all([
        supabase
          .from('imported_invoices')
          .select('*')
          .eq('organization_id', organization.id)
      ...
      logger.debug(`Fetched ${invoices.length} invoices, ${receipts.length} receipts, ${(scannedRows || []).length} bottle_scans`);
      ...
      // Auto-approve pending imports when Trk matches Inv (each split order card checked once per DB row).
      if (!skipAutoApprove) {
  ...
  async function fetchAllScanned() {
    try {
      ...
      const filteredBottleScans = scannedRows || [];
  ...
  async function fetchScannedOnlyOrders() {
    try {
      ...
      // Also check if order has bottle_scans (approval is on import record only)
  ...
  async function fetchBottleInfoForOrder(orderNumber, customerFilter = null, targetCustomer = null) {
    try {
      ...
      // Get all scanned barcodes for this order (single source: bottle_scans)
      ...
      // Filter by customer when viewing scanned-only card (same order can have multiple customers)
      let bottleScansForBarcodes = bottleScans || [];
      if (customerFilter && (customerFilter.customerName || customerFilter.customerId)) {
        ...
        bottleScansForBarcodes = bottleScansForBarcodes.filter(bs => {
          const sn = norm(bs.customer_name || bs.customer);
          const si = norm(bs.customer_id || bs.CustomerID || bs.CustomerId || bs.CustomerListID);
          return (wantName && sn === wantName) || (wantId && si === wantId);
        });
      }
      if (bottleScansForBarcodes.length > 0) {
        bottleScansForBarcodes.forEach(scan => {
        ...
        logger.debug('📊 Found bottle_scans:', bottleScansForBarcodes.length, 'unique barcodes:', Array.from(scannedBarcodes));
      ...
      if (allScannedRows && allScannedRows.length > 0) {
        ...
        if (fallbackScansFiltered.length > 0) {
          ...
          // Also add to bottleScans for matching with bottles (single source: bottle_scans)
          bottleScans = [...bottleScans, ...fallbackScansFiltered];
          
          logger.debug('✅ FALLBACK: Added', scannedBarcodes.size - beforeFallback, 'new barcodes (total:', scannedBarcodes.size, ')');
          logger.debug('✅ FALLBACK: Added', fallbackScansFiltered.length, 'bottle_scans');
        } else {
      ...
      if (scannedBarcodes.size > 0 && organization?.id) {
        ...
        const barcodeVariants = expandBarcodeLookupVariants(barcodeArray);
        ...
        const { data: bottleData, error: bottleError } = await supabase
          .from('bottles')
          .select('*')
          .in('barcode_number', barcodeVariants)
          .eq('organization_id', organization.id);
        ...
        if (bottleError) {
          ...
          bottleData.forEach(bottle => {
            ...
            const scanActionUpper = String(scanAction || '').trim().toUpperCase();
            const shipScanOnThisOrder =
              scanActionUpper === 'OUT' ||
              scanActionUpper === 'SHIP' ||
              scanActionUpper === 'DELIVERY';
            const latestScanIsThisOrderShip =
              !!movementNow &&
              movementNow.event_order_norm === orderNorm &&
              movementNow.status === 'rented';
  ...
  function getDetailedLineItems(data) {
    ...
    const hasScannedOnlyCharacteristics = lineItems.length > 0 && lineItems.every(item => {
      ...
      const hasInvoiceQuantities = !!(item.shipped || item.returned || item.QtyOut || item.QtyIn || item.quantity || item.return_qty || item.ReturnQty);
    ...
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} flexDirection={{ xs: 'column', md: 'row' }} gap={2}>
        <Box>
          ...
          <Typography variant="h4" gutterBottom sx={{ fontWeight: 700, color: 'text.primary', letterSpacing: '-0.03em', mb: 0.5 }}>
        ...
        onSave={async (bottleData) => {
          try {
            if (editBottleDialog.isNew) {
              // Create new bottle
              const barcode = normalizeBottleBarcode(editBottleDialog.barcode);
              const { error } = await supabase
                .from('bottles')
                .insert([{
                  ...bottleData,
              ...
              if (error) {
                if (isDuplicateBarcodeDbError(error)) {
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Enter the order number for scans you rejected by accident. (Note: bottle_scans for that order were removed when rejected; re-scan to add them again.)
          </Typography>
          <TextField
            autoFocus
            fullWidth
    ...
    return (
      <Box sx={{ mt: 2 }}>
        {sortedFilteredInvoices.map((invoice, index) => {
          ...
          return (
<Card
              key={`${invoice.displayId || invoice.id}-${index}`}
              sx={{ 
                mb: 3,
                border: '1px solid #e0e0e0',
                borderRadius: 2,
...
return (
            <Grid item xs={12} sm={6} md={4} key={`${invoice.displayId || invoice.id}-${index}`}>
              <Card
                sx={{
                  height: '100%', 
                  display: 'flex', 
    ...
    return (
      <Box>
        {sortedFilteredInvoices.map((invoice, index) => {
          ...
          return (
            <Box key={`${invoice.displayId || invoice.id}-${index}`} sx={{ display: 'flex', mb: 3 }}>
...
return (
            <Grid item xs={12} sm={6} md={4} key={`${receipt.displayId || receipt.id}-${index}`}>
              <Card
                sx={{
                  height: '100%', 
                  display: 'flex', 
    ...
    return (
      <Box>
        {sortedFilteredReceipts.map((receipt, index) => {
          ...
          return (
            <Box key={`${receipt.displayId || receipt.id}-${index}`} sx={{ display: 'flex', mb: 3 }}>
              <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mr: 2 }}>
                <Box
                  sx={{
                    width: 12,
  ...
  async function confirmApprove(type, row) {
    try {
      ...
      if (typeof row.id === 'string' && row.id.startsWith('scanned_')) {
        ...
        // bottle_scans has no status; approval is on import record only
        logger.debug(`🔄 Checking bottle_scans for order ${orderNumber}, org: ${organization?.id}`);
        ...
        if (checkError) {
          logger.error('❌ Error checking bottle_scans:', checkError);
        } else {
          logger.debug(`📋 Found ${existingScans?.length || 0} bottle_scans for order ${orderNumber}`);
        }
        
        if (existingScans && existingScans.length > 0) {
          logger.debug(`✅ bottle_scans present for order ${orderNumber} (approval tracked on import record only)`);
        } else {
          logger.debug(`ℹ️ No bottle_scans for order ${orderNumber}`);
        }
        ...
        // Approved orders are filtered by import record status, not bottle_scans
      ...
      // Assign bottles to customers BEFORE writing status: 'approved'. If this throws, the order
      // must stay pending — previously the status write below ran first, so a failed/thrown
      // assignment still left the order marked "approved" with bottles never assigned, and the
      // only way to notice/fix it was the now-removed org-wide "Fix delivered assignments" sweep.
      await assignBottlesToCustomer(row);
      ...
      if (orderNumber && organization?.id) {
        ...
        if (targetNorm) {
          ...
          // bottle_scans has no status; approved state is reflected by import record only
        }
      }
  ...
  async function confirmReject(type, row) {
    try {
      ...
      if (typeof row.id === 'string' && row.id.startsWith('scanned_')) {
        ...
        // Delete bottle_scans for this order (scope by org; single source of truth)
  ...
  // Restore accidentally rejected scans (scanned-only): bottle_scans were deleted on reject; no status to restore.
  async function confirmRestoreRejectedScans() {
    ...
    try {
      ...
      setSnackbar('Scanned-only orders use bottle_scans; once rejected, scans were removed. Re-scan to add again.');
      await fetchData();
    } catch (err) {
  ...
  async function checkBottlesAtCustomersForAllOrders(records) {
    try {
      ...
      // Extract all unique order numbers and delivery customer per order
      const orderNumbers = new Set();
      const customerByOrderNorm = new Map();
      records.forEach(record => {
        const data = parseDataField(record.data);
        const orderNumber = getOrderNumber(data);
        ...
        const norm = normalizeImportOrderNum(trimmed);
        const customerInfo = {
          id: getCustomerId(data),
          name: getCustomerInfo(data),
        };
        customerByOrderNorm.set(norm, customerInfo);
        customerByOrderNorm.set(trimmed, customerInfo);
      });
      ...
      if (allBarcodes.size > 0) {
        ...
        // Create a map of barcodes to at-customer status and assignment (for same-customer checks).
        ...
        (allBottles || []).forEach(bottle => {
          ...
          bottleCustomerMap.set(bottle.barcode_number, {
            atCustomer: isAtCustomer,
            customerName: bottle.customer_name,
            assignedCustomer: bottle.assigned_customer,
            movementCustomerName: movementNow?.customer_name || null,
            movementAssignedCustomer: movementNow?.assigned_customer || null,
          });
        });
        
        // Flag orders where a ship bottle is at a different customer (not the delivery customer).
        barcodesByOrder.forEach((barcodes, orderNum) => {
          const orderNorm = normalizeImportOrderNum(orderNum);
          const orderCustomer = customerByOrderNorm.get(orderNorm) || customerByOrderNorm.get(String(orderNum).trim());
          for (const barcode of barcodes) {
            ...
            const bottleInfo = bottleCustomerMap.get(barcode);
            if (!bottleInfo?.atCustomer) continue;
            ...
            if (
              ...
              isSameCustomerIdentity(
                orderCustomer.id,
                orderCustomer.name,
                bottleInfo.customerName,
                bottleInfo.assignedCustomer,
                bottleInfo.movementCustomerName,
                bottleInfo.movementAssignedCustomer
              )
            ) {
              continue;
            }
  ...
  async function autoApproveIfQuantitiesMatch(record) {
    try {
      ...
      // Assign bottles BEFORE writing the approved/verified status — this runs unattended, so if it
      // throws (bad barcode, no CustomerListID, etc.) the record must stay pending for a human to see
      // in the list, not silently sit "approved" with bottles never assigned.
  ...
  async function collectBarcodesForOrder(orderNumber, recordRows = null, recordData = null) {
    ...
    if (orderVariants.length > 0) {
      ...
      if (bottleScans && bottleScans.length > 0) {
        ...
        bottleScans.forEach(scan => {
          ...
          const time = new Date(scan.timestamp || scan.created_at || 0).getTime();
          const existing = bottleScanMap.get(norm);
          if (!existing || time >= existing.time) {
            bottleScanMap.set(norm, { mode: scan.mode, isDelivered: isDeliveredScan, time, raw: String(barcode).trim() });
          }
        });
        bottleScanMap.forEach((scanInfo) => {
          const bc = scanInfo.raw;
          if (scanInfo.isDelivered) shippedBarcodes.add(bc);
          else returnedBarcodes.add(bc);
        });
      }
    }
    ...
    // RETURN takes precedence over SHIP for the same barcode (use normalized comparison)
    const normalizedReturnSet = new Set([...returnedBarcodes].map(normalizeBarcode).filter(Boolean));
    for (const bc of [...shippedBarcodes]) {
  ...
  async function assignBottlesToCustomer(record) {
    try {
      ...
      // Actual scan date per barcode, so rentals.rental_start_date reflects when the bottle was
      // really delivered, not the day someone got around to approving the order. Using "today" here
      // created a second, later-dated "Delivery" entry in movement history alongside the real scan
      // (e.g. scanned 7/29, approved 8/3 -> two rows for the same delivery).
      const shipDateByBarcode = {};
      (bottleScanRows || []).forEach((scan) => {
        const modeUpper = String(scan?.mode || '').toUpperCase();
        if (modeUpper !== 'SHIP' && modeUpper !== 'DELIVERY') return;
        const norm = normalizeBarcodeAssign(scan?.bottle_barcode);
        if (!norm) return;
        const iso = scan?.timestamp || scan?.created_at;
        if (!iso) return;
        const dateOnly = String(iso).slice(0, 10);
        const existing = shipDateByBarcode[norm];
        if (!existing || dateOnly > existing) shipDateByBarcode[norm] = dateOnly;
      });
      ...
      if (retArr.length > 0 && organization?.id) {
        ...
        if (bottleRows && bottleRows.length < allBc.length) {
          const missing = allBc.filter(bc => !bottleMap.has(bc));
          if (missing.length > 0) {
      ...
      const rpcResult = await bottleAssignmentService.assignBottles({
        ...
        shipBarcodeDates: shipDateByBarcode,
      });
      ...
      if (rpcResult.success) {
        ...
        // Barcodes intentionally skipped because a newer scan already moved them to a different order —
        // that's correct, expected behavior (not a failure) and must not block this order's approval.
        const staleSkipped = Number(d.stale_skipped || 0);
        logger.debug(
          `RPC assignment succeeded: ${shippedCount} shipped, ${d.returned || 0} returned, ${d.skipped || 0} skipped, ${d.created || 0} created`,
        );
        const preReconcileShortfall = Math.max(0, shipArr.length - shippedCount - alreadyOnCustomer - staleSkipped);
        ...
        // Self-heals the most common cause of a shortfall: bottle still shows a prior customer because
        // its return was never scanned. Must run BEFORE we decide whether to fail the approval, since
        // it can resolve every barcode preReconcileShortfall counted as unassigned.
        ...
        const genuineShortfall = Math.max(0, preReconcileShortfall - reconcileWarnings.length);
        if (genuineShortfall > 0) {
          const warnDetail =
            (Array.isArray(d.warnings) && d.warnings.length && d.warnings.join('; ')) ||
            (Array.isArray(d.errors) && d.errors.length && d.errors.join('; ')) ||
            'Check barcodes exist in Assets and the customer has a CustomerListID.';
          logger.warn('assignBottlesToCustomer: delivery scan(s) could not be assigned', {
            orderNumber,
            shipArr,
            genuineShortfall,
            staleSkipped,
          ...
          // Throw instead of warning-and-continuing: the caller (confirmApprove /
          // autoApproveIfQuantitiesMatch) must not mark this order approved while a delivered bottle
          // has no renter — there is no longer an org-wide sweep to catch it after the fact.
          throw new Error(
            `${genuineShortfall} of ${shipArr.length} delivered bottle(s) could not be assigned to ${newCustomerName} (${warnDetail}). Fix the barcode/customer issue and try again.`,
          );
        }
  ...
  // Normalizes an order/invoice date to YYYY-MM-DD, falling back to today when absent/unparseable.
  function toIsoDateForInlineAssign(value) {
  ...
  async function assignBottlesToCustomerInline(record, newCustomerName, newCustomerId, orderNumber, shippedBarcodes, returnedBarcodes, rows, effectiveDate = null) {
    ...
    for (const barcode of returnedBarcodes) {
      ...
      const alreadyHasReturnScan = await hasExistingReturnScanForBarcode(
        supabase,
        organization?.id,
        bottle.barcode_number || barcode,
        orderNumber,
      );
      if (!alreadyHasReturnScan) {
      ...
      if (isAtHome) {
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            status: 'rented',
            rental_start_date: toIsoDateForInlineAssign(effectiveDate),
            last_verified_order: orderNumber,
          })
          .eq('id', bottle.id);
        ...
        await createRentalRecord(bottle, newCustomerName, newCustomerId, null, orderNumber, effectiveDate);
  ...
  async function createRentalRecord(bottle, customerName, customerId, row, orderNumber = null, rentalStartDate = null) {
    try {
      ...
      const rentalResult = await createOpenRentalForShippedBottle(supabase, organization.id, {
        ...
        customerName,
        orderNumber,
        ...(rentalStartDate ? { rentalStartDate: toIsoDateForInlineAssign(rentalStartDate) } : {}),
      });
  ...
  async function createDNSRentalRecord(record, row, customerName, customerId, orderNumber, assignmentSuccesses = [], assignmentWarnings = []) {
    ...
    try {
      ...
      if (error) {
        ...
        logger.debug('Created DNS rental for', productCode);
      }
    } catch (err) {
      logger.error('createDNSRentalRecord error:', err);
    }
  }
  ...
  async function createRNBRentalRecord(customerName, customerId, orderNumber, productCode, scannedBarcode = null) {
    ...
    try {
      ...
      if (error) logger.error('Error creating RNB rental:', error);
  ...
  async function handleBulkReject(records) {
    ...
    for (const record of records) {
      ...
      if (typeof record.id === 'string' && record.id.startsWith('scanned_')) {
        ...
        // Delete bottle_scans for this order (single source of truth; no status to set)
        logger.debug('Deleting bottle_scans for rejected order:', orderNumber);
        const { error: bottleScanError } = await supabase
          .from('bottle_scans')
          .delete()
          .eq('order_number', orderNumber)
          .eq('organization_id', organization?.id);
        if (bottleScanError) {
          logger.error('Error deleting bottle_scans for order:', orderNumber, bottleScanError);
        } else {
          logger.debug('Deleted bottle_scans for order:', orderNumber);
        }
        continue;
      }
      ...
      if (checkError) {
        ...
        // Check bottle_scans and other tables
  ...
  async function handleBulkExport(records) {
    if (!records || records.length === 0) {
      setSnackbar('No records selected to export');
      return;
    }
    // Create CSV data
  ...
  function getRecordDate(data) {
    ...
    const formatRaw = (raw) => {
      ...
      return formatImportDateForDisplay(raw, { timeZone: displayTimezone });
    };
    if (data.date || data.invoice_date || data.receipt_date) {
      return formatRaw(data.date || data.invoice_date || data.receipt_date);
    }
    if (data.rows && data.rows.length > 0) {
      const firstRow = data.rows[0];
      if (firstRow.date || firstRow.invoice_date || firstRow.receipt_date) {
        return formatRaw(firstRow.date || firstRow.invoice_date || firstRow.receipt_date);
      }
    }
    return '';
  }
  ...
  function getScannedQty(orderNum, productCode, type, invoiceRecord = null) {
    const normalizeProductCode = (pc) => (pc == null || pc === '') ? '' : String(pc).trim().replace(/\s+/g, '');
    ...
    const resolvedNorm = normalizeProductCode(resolvedProductCode);
