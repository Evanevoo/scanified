  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#fff', py: 8, borderRadius: 0, overflow: 'visible' }}>
      <Paper elevation={0} sx={{ maxWidth: 1100, mx: 'auto', p: { xs: 2, md: 5 }, borderRadius: 0, boxShadow: '0 2px 12px 0 rgba(16,24,40,0.04)', border: '1px solid #eee', bgcolor: '#fff', overflow: 'visible' }}>
        ...
        {preview.length === 0 && (
          <Alert severity="warning" sx={{ mb: 3 }}>No data was parsed from your file. Please check your file format and column headers.</Alert>
        )}
        ...
        {loading && (
          <Box mb={4} display="flex" justifyContent="center" alignItems="center">
            <Typography variant="h6" color="primary">Processing file, please wait...</Typography>
          </Box>
        )}
        {preview.length > 0 && (
          <Box mt={4}>
            ...
            <Paper variant="outlined" sx={{ overflowX: 'auto', borderRadius: 3, border: '1px solid #e3e7ef', boxShadow: 'none', width: '100%', minWidth: 1200, maxWidth: '100vw' }}>
        ...
        {columns.length > 0 && (
          <Alert severity="info" sx={{ mb: 2 }}>
            <div><b>Detected columns:</b> {columns.join(', ')}</div>
            {preview.length > 0 && <div><b>First data row:</b> {columns.map(col => preview[0][col]).join(' | ')}</div>}
          </Alert>
        )}
