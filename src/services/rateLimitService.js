...
/**
 * Rate Limiter Service for Web App
 * Synced with mobile app's RateLimitService.ts
 * Prevents excessive API calls and protects against abuse
 */

// Rate limit configurations - synced with mobile app
...
class RateLimitService {
   ...
   * @param {string} identifier - Unique identifier (e.g., user ID, session ID)
   * @param {string} endpoint - The endpoint or action being rate limited
   * @param {string} limitType - Type of limit to apply (default, auth, read, write, etc.)
   * @returns {Object} - { allowed: boolean, retryAfter?: number, remaining?: number }
   */
  checkLimit(identifier, endpoint, limitType = 'default') {
   ...
   * @param {string} endpoint - The endpoint or action
   * @param {Function} operation - The async operation to execute
   * @param {string} limitType - Type of limit to apply
   * @returns {Promise} - Result with success status
   */
  async withRateLimit(identifier, endpoint, operation, limitType = 'default') {
   ...
   */
  getStatus(identifier, endpoint, limitType = 'default') {
    const key = `${identifier}-${endpoint}`;
    const config = DEFAULT_LIMITS[limitType] || DEFAULT_LIMITS.default;
    const entry = this.requests.get(key);
