...
function Customers({ profile }) {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '100%', mx: 0, mt: 4 }}>
      <Paper elevation={3} sx={{ borderRadius: 4, p: 0, width: '100%', maxWidth: '100%' }}>
        <Box display="flex" justifyContent="flex-end" alignItems="center" gap={2} sx={{ p: 3, pb: 0 }}>
          ...
          <Button variant="contained" disabled sx={{ fontWeight: 700, borderRadius: 8, px: 3, bgcolor: '#e0e0e0', color: '#444', boxShadow: 'none' }}>
            Delete Selected
          </Button>
              ...
              <TableBody>
                {paginatedCustomers.map((c, idx) => (
                  <TableRow key={c.CustomerListID} sx={{ borderBottom: '1.5px solid #f0f0f0' }}>
                    ...
                    <TableCell sx={{ fontWeight: 700, color: '#1976d2', cursor: 'pointer' }}>{c.name}</TableCell>
                    <TableCell>{c.CustomerListID}</TableCell>
                    <TableCell>{c.contact_details}</TableCell>
                    <TableCell>{c.phone}</TableCell>
                    <TableCell>{c.assets || 0}</TableCell>
                    <TableCell>
                      <Button variant="text" sx={{ color: '#1976d2', fontWeight: 700, textTransform: 'none', minWidth: 0, px: 1 }} onClick={() => handleEdit(c)}>Edit</Button>
