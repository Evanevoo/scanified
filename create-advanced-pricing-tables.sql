-- Advanced Pricing System Tables
-- These tables support bracket-based pricing, customer-specific rates, and demurrage calculations
...
(VALUES 
  ('propane', 1, 2.50, 15.00, 45.00),
  ('oxygen', 1, 4.00, 25.00, 75.00),
  ('nitrogen', 1, 3.50, 20.00, 60.00),
  ('acetylene', 1, 5.00, 30.00, 90.00)
) AS gas_types(type, min_qty, daily, weekly, monthly)
WHERE NOT EXISTS (
  SELECT 1 FROM pricing_tiers pt 
  WHERE pt.organization_id = o.id 
  AND pt.gas_type = gas_types.type
);

-- Insert some default demurrage rules
INSERT INTO demurrage_rules (organization_id, gas_type, grace_period_days, daily_penalty)
SELECT 
  o.id,
  gas_types.type,
  gas_types.grace_days,
  gas_types.penalty
FROM organizations o,
(VALUES 
  ('propane', 30, 5.00),
  ...
  ('nitrogen', 21, 6.00),
  ('acetylene', 7, 10.00)
) AS gas_types(type, grace_days, penalty)
WHERE NOT EXISTS (
  SELECT 1 FROM demurrage_rules dr 
  WHERE dr.organization_id = o.id 
  AND dr.gas_type = gas_types.type
);
...
CREATE TRIGGER update_customer_pricing_updated_at BEFORE UPDATE ON customer_pricing FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_demurrage_rules_updated_at BEFORE UPDATE ON demurrage_rules FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
