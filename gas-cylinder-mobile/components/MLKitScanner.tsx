 ...
 * 
 * OPTIMIZATION FOR 1D BARCODES ON PAPER (2025-01-24):
 * - Removed 2D barcode types (qr, datamatrix, pdf417, aztec) to reduce false positives
 * - Added validation to reject short numeric codes (< 6 digits) that might be OCR errors
 * - Focus on 1D barcode types: code128, code39, codabar, ean13, ean8, upc_a, upc_e, code93, itf14
 * 
 * TO REVERT: Search for "OPTIMIZED FOR 1D BARCODES" and "VALIDATION FOR 1D BARCODES" comments
 */
...
const MLKitScanner: React.FC<MLKitScannerProps> = ({
  ...
  const barcodeFoundRef = useRef(false); // Stop OCR after barcode found
  ...
  const startOcrTimer = useCallback(() => {
    // Don't start OCR if barcode was already found
  ...
  const stopOcr = useCallback(() => {
    // Completely stop OCR - used when barcode is found
  ...
  const resetOcrTimer = useCallback(() => {
    ...
    // Only restart if barcode hasn't been found
    if (enabled && !batchMode && !barcodeFoundRef.current) {
      startOcrTimer();
    }
  ...
  const tryOcr = useCallback(async () => {
    // Don't run OCR if barcode was already found
    if (barcodeFoundRef.current) {
      return;
    }
    ...
    try {
      ...
      // Calculate crop region to match the scan frame
      // The frame is positioned at FRAME_TOP from top, centered horizontally
      const photoWidth = photo.width || 1920;
      const photoHeight = photo.height || 1080;
      
      // Calculate scale factors between photo and screen
      ...
      // Add some padding to capture a bit more area (30% larger)
      const padding = 0.3;
      const expandedWidth = FRAME_WIDTH * (1 + padding);
      const expandedHeight = FRAME_HEIGHT * (1 + padding);
      const expandedLeft = Math.max(0, frameLeft - (expandedWidth - FRAME_WIDTH) / 2);
      const expandedTop = Math.max(0, frameTop - (expandedHeight - FRAME_HEIGHT) / 2);
      
      // Convert to photo coordinates
      const cropOriginX = Math.round(expandedLeft * scaleX);
      const cropOriginY = Math.round(expandedTop * scaleY);
      const cropWidth = Math.round(expandedWidth * scaleX);
      const cropHeight = Math.round(expandedHeight * scaleY);
      
      console.log(`📐 Cropping: ${cropOriginX},${cropOriginY} ${cropWidth}x${cropHeight} (photo: ${photoWidth}x${photoHeight})`);
      
      // Crop the image to the frame area
      let imageToRecognize = photo.uri;
      try {
        const cropped = await ImageManipulator.manipulateAsync(
          photo.uri,
          [{
            crop: {
              originX: Math.max(0, Math.min(cropOriginX, photoWidth - cropWidth)),
              originY: Math.max(0, Math.min(cropOriginY, photoHeight - cropHeight)),
              width: Math.min(cropWidth, photoWidth),
              height: Math.min(cropHeight, photoHeight),
            }
          }],
          { compress: 0.8, format: ImageManipulator.SaveFormat.JPEG }
        );
        imageToRecognize = cropped.uri;
        console.log('✂️ Image cropped to frame area');
      } catch (cropError) {
        console.log('⚠️ Crop failed, using full image:', cropError);
        // Fall back to full image if cropping fails
      }

      console.log('🔍 Running text recognition on cropped area...');
      const result = await TextRecognition.recognize(imageToRecognize);
      lastOcrTimeRef.current = Date.now();

      if (result?.text) {
        console.log('📝 OCR found text:', result.text.substring(0, 200));
        
        // Common headers and labels to exclude
        const excludedTerms = [
          'sales receipt', 'sales receipts', 'receipt', 'receipts',
          'ship to', 'ship from', 'bill to', 'sold to',
          'amount', 'rate', 'quantity', 'qty', 'total', 'subtotal',
          'lot num', 'lot number', 'invoice', 'order',
          'date', 'due date', 'terms', 'po number',
          'description', 'item', 'price', 'tax',
          'n/a', 'na', 'none'
        ];
        
        const allLines = result.text.split('\n');
        
        // Look for customer name after "Ship To" line
        let shipToIndex = allLines.findIndex(line => 
        ...
        // If we found "Ship To", prioritize lines after it
        if (shipToIndex >= 0) {
          // Check lines after "Ship To" (usually customer name is next)
          for (let i = shipToIndex + 1; i < Math.min(shipToIndex + 4, allLines.length); i++) {
            const line = allLines[i].trim().toUpperCase();
            if (line.length >= 3 && 
                line.length <= 50 &&
                /^[A-Z\s&'.-]+$/.test(line) &&
                !excludedTerms.some(term => line.toLowerCase() === term)) {
              console.log('👤 Found name after Ship To:', line);
              possibleNames.push(line);
              break; // Take first valid name after Ship To
            }
          }
        }
        
        // Also check all lines for company-like names (multi-word, all caps)
        if (possibleNames.length === 0) {
          for (const line of allLines) {
            ...
            // Skip excluded terms
            if (excludedTerms.some(term => cleaned.toLowerCase() === term)) {
              continue;
            }
            
            // Look for company names: multiple words, mostly letters
            if (cleaned.length >= 5 && 
                cleaned.length <= 50 &&
                /^[A-Za-z][A-Za-z\s&'.-]+$/.test(cleaned) &&
                cleaned.split(/\s+/).length >= 2) {
              possibleNames.push(upper);
            }
          }
        }
        
        if (possibleNames.length > 0) {
          console.log('👤 Possible customer names found:', possibleNames);
      ...
      isProcessingOcrRef.current = false;
      // Only continue OCR if barcode hasn't been found
      if (!barcodeFoundRef.current) {
        setOcrStatus('scanning');
        startOcrTimer();
      }
  ...
  const handleBarcode = useCallback((event: any) => {
    ...
    // Stop OCR completely since barcode was found
    stopOcr();
    setOcrStatus('idle');
    setFoundText('');
  ...
  }, [enabled, batchMode, onBarcodeScanned, stopOcr]);
